import { Controller, Param, UseGuards, Request, Get, Post, Delete } from "@nestjs/common";
...
export class WishListController {
    ...
    @Delete('remove-item/:bookId')
    @UseGuards(AuthGuard)
    async removeItemFromWishList( @Param('bookId') bookId: number, @Request() req ) {
        const uId = req.user.uid;
        const result = this.wishListService.removeItem(bookId, uId);
        return result;
    }
}