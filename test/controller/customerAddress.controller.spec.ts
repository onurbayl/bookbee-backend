...
describe('CustomerAddressController', () => {
    ...
    describe('/api/v1/customerAddress/get-address', () => {
        ...
        it('Success', async () => {
            ...
            .get('/api/v1/customerAddress/get-address')
            .set('Authorization', `Bearer ${bearerToken}`)
            .expect(200);

            expect(response.body).toBeDefined();
            expect(response.body.addressInfo).toEqual('Banana Street, Minion Building');
        });

    });

    describe('/api/v1/customerAddress/add-address', () => {
        ...
        it('Success', async () => {
            ...
            newAddress.addressInfo = 'Banana';

            const addAddress = await request(app.getHttpServer())
            .post('/api/v1/customerAddress/add-address')
            .set('Authorization', `Bearer ${bearerToken}`)
            .send(newAddress)
            .expect(201);
            ...
            expect(response.body).toBeDefined();
            expect(response.body.addressInfo).toEqual('Banana');
        });
